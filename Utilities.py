...
def transform_transition_matrix(subject_similarity_matrix, n):
    transition_matrix = np.zeros((40, 40), dtype=np.float32)

    for i, row in enumerate(subject_similarity_matrix):
        ...
        for j, score in enumerate(row):
            if i != j:
                heapq.heappush(heap_list, (-score, j))
            if len(heap_list) > n:
...
def perform_ascos_similarity(transition_matrix):
